  ...
  try {
    ...
    for (const userDoc of usersSnapshot.docs) {
      try {
        ...
        const stats = await getUserWeeklyStats(db, userId);
...
/**
 * Get user's stats for the past week
 */
async function getUserWeeklyStats(db: FirebaseFirestore.Firestore, userId: string): Promise<{
  ...
  try {
    // Get page views for user's pages in the last week
    ...
    // Get earnings this week from writer balance
