...
export async function GET(request: NextRequest) {
  try {
    ...
    // For dev auth system, if there's no sessionId, just validate the simpleUserSession
    if (!sessionId) {
      // In development, if we have a valid simpleUserSession, consider it valid
      if (process.env.NODE_ENV === 'development' && userId) {
        console.log('[Session Validation] Dev auth session valid for:', userId);
        return NextResponse.json({ valid: true });
      }
      console.log('[Session Validation] No session ID found, marking as invalid');
      return NextResponse.json({ valid: false, reason: 'No session ID found' });
    }
