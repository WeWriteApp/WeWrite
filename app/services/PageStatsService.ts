import { getDatabase, ref, onValue, set, get, increment, serverTimestamp, Database, Unsubscribe } from 'firebase/database';
...
class PageStatsService {
   ...
   */
  subscribeToRecentChanges(pageId: string, callback: (count: number) => void): Unsubscribe | null {
   ...
   */
  subscribeToEditorsCount(pageId: string, callback: (count: number) => void): Unsubscribe | null {
    if (!pageId || !callback) return null;
   ...
   */
  async getTotalReadersCount(pageId: string): Promise<number> {
    if (!pageId) return 0;
   ...
   */
  unsubscribeFromStats(pageId: string): void {
    if (!pageId) return;
