    ...
    <div className={cn("animate-pulse space-y-6", className)}>
      {showHeader && (
        <div className="space-y-4">
          <div className="h-8 bg-muted/20 rounded-md w-3/4"></div>
        ...
        {Array(count).fill(0).map((_, i) => (
          <div key={i} className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="h-4 bg-muted/20 rounded-md w-full"></div>
              <div className="h-4 bg-muted/20 rounded-md w-5/6"></div>
              <div className="h-4 bg-muted/20 rounded-md w-4/6"></div>
            </div>
            <div className="flex space-x-2 pt-2">
              <div className="h-3 bg-muted/20 rounded-md w-16"></div>
              <div className="h-3 bg-muted/20 rounded-md w-12"></div>
            </div>
          </div>
        ))}
      </div>
...
export function UserBioSkeleton({ className }: UserBioSkeletonProps) {
  return (
    <div className={cn("animate-pulse space-y-4", className)}>
      <div className="h-6 bg-muted/20 rounded-md w-1/3"></div>
      <div className="space-y-2">
        <div className="h-4 bg-muted/20 rounded-md w-full"></div>
        <div className="h-4 bg-muted/20 rounded-md w-5/6"></div>
        <div className="h-4 bg-muted/20 rounded-md w-4/6"></div>
  ...
  return (
    <div className={cn("animate-pulse space-y-4", className)}>
      ...
      <div className="space-y-2">
        ...
        <div className="h-4 bg-muted/20 rounded-md w-5/6"></div>
        <div className="h-4 bg-muted/20 rounded-md w-4/6"></div>
        <div className="h-4 bg-muted/20 rounded-md w-3/6"></div>
      </div>
    </div>
  );
}
      ...
      <div className="space-y-4">
        {Array(paragraphCount).fill(0).map((_, i) => (
          <div key={i} className="space-y-2">
            ...
            <div className="h-4 bg-muted/20 rounded-md w-5/6"></div>
            <div className="h-4 bg-muted/20 rounded-md w-4/6"></div>
            {i % 2 === 0 && <div className="h-4 bg-muted/20 rounded-md w-3/6"></div>}
          </div>
        ))}
      </div>
    </div>
...
export function TableSkeleton({ className, rowCount = 5, columnCount = 4 }: TableSkeletonProps) {
  return (
    <div className={cn("animate-pulse", className)}>
      {/* Header */}
      <div className="flex border-b-only pb-2 mb-2">
