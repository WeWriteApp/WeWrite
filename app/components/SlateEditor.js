...
const SlateEditor = forwardRef(({ initialContent, onContentChange, onInsert, onDiscard, onSave }, ref) => {
  ...
  const handleKeyDown = (event, editor) => {
    ...
    // Check for Backspace or Delete key to handle link deletion
    if ((event.key === 'Backspace' || event.key === 'Delete') && editor.selection) {
      ...
      // Check if we're at the edge of a link node
      const [linkNode, linkPath] = Editor.above(editor, {
        at: editor.selection,
        match: n => !Editor.isEditor(n) && SlateElement.isElement(n) && n.type === 'link'
      }) || [null, null];

      if (linkNode) {
        // We're at a link, delete the entire link
        event.preventDefault();
  ...
  const handleKeyDown = (e) => {
    try {
      ...
      if (e.key === 'Backspace' || e.key === 'Delete') {
        ...
        try {
          // First, select the node to ensure proper cursor positioning after deletion
...
const LinkEditor = ({ position, onSelect, setShowLinkEditor, initialText = '', initialPageId = null, initialPageTitle = '' }) => {
  ...
  const handlePageSelect = (item) => {
    ...
    if (item.type === 'user') {
      ...
      onSelect({
        text: item.title, // Always use page title as text for consistency
