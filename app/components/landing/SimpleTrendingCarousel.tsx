  ...
  useEffect(() => {
    const fetchTrendingPages = async () => {
      try {
        ...
        // Use the same function as the logged-in home page
        const pages = await getTrendingPages(limit);

        if (!pages || !Array.isArray(pages)) {
          console.error('SimpleTrendingCarousel: Invalid trending pages data received', pages);
          setError('Invalid data received from server');
          setTrendingPages([]);
        } else if (pages.length === 0) {
          console.log('SimpleTrendingCarousel: No trending pages found');
          setError(null);
          setTrendingPages([]);
        } else {
          // Limit the number of pages to reduce memory usage
          const limitedPages = pages.slice(0, Math.min(10, pages.length));
