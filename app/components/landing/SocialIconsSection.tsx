...
const socialLinks: SocialLink[] = [
  {
    ...
    renderIcon: ({ size, className }) => <Icon name="Instagram" size={size} className={className} />,
  },
  {
    name: 'X',
    url: 'https://x.com/WeWriteApp',
    renderIcon: ({ size, className }) => <XLogo size={size} className={className} />,
  },
  {
    name: 'GitHub',
    url: 'https://github.com/WeWriteApp/WeWrite',
    renderIcon: ({ size, className }) => <GitHubLogo size={size} className={className} />,
  },
];
        ...
        <div className="flex items-center justify-center gap-4">
          {socialLinks.map((link) => (
            <a
              ...
              aria-label={`Visit WeWrite on ${link.name}`}
            >
              {link.renderIcon({ size: 20 })}
            </a>
          ))}
        </div>
      </div>
