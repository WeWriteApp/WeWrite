  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        console.log('Page activity - Raw page versions from database:', pageVersions);
        ...
        if (!pageVersions || pageVersions.length === 0) {
          ...
          // Create multiple fallback versions to show in activity
          versionsToUse = [
            {
              id: 'fallback-1',
              createdAt: new Date(),
        ...
        const activityItems = sortedVersions.map((version, index) => {
          ...
          return {
            ...
            isActivityContext: true, // Flag to indicate this is from activity page
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('Activity page - setting activities:', activityItems.length, 'items');
        }
    ...
    return (
      <div className="p-4 max-w-4xl mx-auto">
        ...
        <div className="text-destructive text-center p-8 border border-destructive/20 rounded-lg bg-destructive/5">
          ...
          <p className="text-sm mt-2 text-muted-foreground">Unable to load page activity. Please try again later.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-4 max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-2xl font-semibold">Page Activity</h2>
          </div>

          {activities.length === 0 ? (
            <div className="text-center p-8 border rounded-md">
              <p className="text-muted-foreground">No activity available for this page</p>
            </div>
          ) : (
