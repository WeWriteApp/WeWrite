...
interface SEOReturn {
  ...
  validateSEO: () => Promise<ValidationResult>;
  autoOptimize: () => void;
 ...
 */
export function useSEO(options: SEOOptions = {}): SEOReturn {
   ...
   */
  const updateMetaTags = useCallback((metaData: MetaData): void => {
   ...
   */
  const generateMetaData = useCallback((): MetaData => {
   ...
   */
  const validateSEO = useCallback(async (): Promise<ValidationResult> => {
    if (typeof window === 'undefined') return { score: 0, issues: [], recommendations: [] };
   ...
   */
  const trackPerformance = useCallback((): void => {
   ...
   */
  const autoOptimize = useCallback((): void => {
