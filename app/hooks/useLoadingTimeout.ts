...
interface LoadingTimeoutReturn {
  ...
  resetTimeout: () => void;
  /** Function to force the loading to complete */
  forceComplete: () => void;
  /** Whether the loading is stalled (loading and timed out) */
  isStalled: boolean;
...
export function useLoadingTimeout(
...
): LoadingTimeoutReturn {
  const [isTimedOut, setIsTimedOut] = useState<boolean>(false);
  const [startTime, setStartTime] = useState<number | null>(null);
  const [elapsedTime, setElapsedTime] = useState<number>(0);
  const intervalRef = useRef<NodeJS.Timeout | null>(null);
  const maxTimeRef = useRef<number>(timeoutMs * 3); // Maximum time before auto-recovery
  ...
  const hasRecoveredRef = useRef<boolean>(false); // Track if we've already recovered
  ...
  const forceComplete = useCallback((): void => {
