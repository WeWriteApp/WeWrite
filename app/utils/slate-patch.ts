...
// Types
interface DOMPoint extends Array<any> {
  0: Node;
  1: number;
}
...
// Extend window interface for global ReactEditor
declare global {
  interface Window {
    ReactEditor?: {
      toSlatePoint?: (editor: SlateEditor, domPoint: DOMPoint) => SlatePoint;
      toDOMNode?: (editor: SlateEditor, node: SlateNode) => Node | null;
      toDOMPoint?: (editor: SlateEditor, point: SlatePoint) => DOMPoint | null;
    };
    applySlatePatches?: () => void;
    __webpack_require__?: {
 ...
 */
export function applySlatePatches(): void {
 ...
 */
function applyToSlatePointPatch(): void {
  // Wait for the ReactEditor to be available
  const checkAndPatch = (): boolean => {
    ...
    if (window.ReactEditor && window.ReactEditor.toSlatePoint) {
      ...
      window.ReactEditor.toSlatePoint = (editor: SlateEditor, domPoint: DOMPoint): SlatePoint => {
        try {
 ...
 */
function applyToDOMNodePatch(): void {
  const checkAndPatch = (): boolean => {
    if (window.ReactEditor && window.ReactEditor.toDOMNode) {
      const originalToDOMNode = window.ReactEditor.toDOMNode;

      window.ReactEditor.toDOMNode = (editor: SlateEditor, node: SlateNode): Node | null => {
        try {
 ...
 */
function applyToDOMPointPatch(): void {
  const checkAndPatch = (): boolean => {
    if (window.ReactEditor && window.ReactEditor.toDOMPoint) {
      const originalToDOMPoint = window.ReactEditor.toDOMPoint;

      window.ReactEditor.toDOMPoint = (editor: SlateEditor, point: SlatePoint): DOMPoint | null => {
        try {
 ...
 */
function applyReactEditorPatches(): void {
  try {
 ...
 */
function patchReactEditorMethods(ReactEditor: any): void {
 ...
 */
function suppressSlateErrors(): void {
