            ...
            <Table className="w-full text-xs sm:text-sm">
              <TableHeader>
                <TableRow className="border-b border-border bg-muted/30">
                  <TableHead className="text-left py-2 px-2">Subscriber</TableHead>
              ...
              <TableBody>
                {stripeSubscriptions.subscribers.map((sub) => (
                  ...
                  >
                    ...
                    <TableCell kind="currency" className={`py-2 px-2 ${sub.subscriptionAmountCents === 0 ? 'opacity-30' : ''}`}>
                      {formatUsdCents(sub.subscriptionAmountCents)}
                    </TableCell>
                    <TableCell kind="currency" className={`py-2 px-2 ${sub.allocatedCents === 0 ? 'opacity-30' : ''}`}>
                      {formatUsdCents(sub.allocatedCents)}
                    </TableCell>
                    <TableCell kind="currency" className={`py-2 px-2 ${sub.overspentUnfundedCents === 0 ? 'opacity-30' : 'text-red-600 dark:text-red-400'}`}>
                      {formatUsdCents(sub.overspentUnfundedCents)}
                    </TableCell>
                    <TableCell kind="currency" className={`py-2 px-2 ${sub.unallocatedCents === 0 ? 'opacity-30' : 'text-green-700 dark:text-green-400'}`}>
              ...
              <TableFooter>
                <TableRow className="font-bold bg-muted/50">
                  ...
                  <TableCell kind="currency" className={`py-2 px-2 ${stripeSubscriptions.subscribers.reduce((sum, s) => sum + s.overspentUnfundedCents, 0) > 0 ? 'text-red-600 dark:text-red-400' : 'opacity-30'}`}>
