...
const NODE_DETAILS: Record<string, NodeDetailData> = {
  ...
  'financial-services': {
    ...
    files: [
      ...
      { path: 'app/services/payoutService.ts', description: 'Payout request and processing' },
      { path: 'app/services/payoutLimitService.ts', description: 'Fraud protection and payout limits' },
    ...
    functions: [
      ...
      { name: 'validatePayoutLimits()', description: 'Verify payout limits and fraud protection' },
      { name: 'requestPayout()', description: 'Process payout requests with approval workflow' },
    ],
  ...
  'groups-services': {
    title: 'Groups System',
    description: 'Group creation, membership, and group page management',
    details: [
      'Group CRUD with slug-based URLs (/g/[slug])',
      'Member management with roles (owner, admin, member)',
      'Invitation system (invite by username, accept/decline)',
      'Group page association (add/remove pages from groups)',
      'Private group access control for members-only content',
      'Feature-flagged under "groups" (auto-enabled for admins)',
    ],
    files: [
      { path: 'app/types/groups.ts', description: 'Group, GroupMember, GroupInvitation types' },
      { path: 'app/firebase/database/groups.ts', description: 'Firestore CRUD for groups and invitations' },
      { path: 'app/api/groups/route.ts', description: 'Create and list groups' },
      { path: 'app/api/groups/[id]/route.ts', description: 'Group detail, update, delete' },
      { path: 'app/api/groups/[id]/members/route.ts', description: 'Member listing and invitations' },
      { path: 'app/api/groups/[id]/pages/route.ts', description: 'Group page management' },
      { path: 'app/api/groups/invitations/route.ts', description: 'Accept/decline invitations' },
    ...
    functions: [
      ...
      { name: 'addGroupMember()', description: 'Add member to group subcollection' },
      { name: 'checkPageAccess()', description: 'Private group page access control' },
      { name: 'createInvitation()', description: 'Send group invitation' },
      { name: 'distributeAmongMembers()', description: 'Split earnings by fund distribution' },
      { name: 'getGroupFundDistribution()', description: 'Get group percentage splits' },
    ],
    upstream: ['firebase'],
    downstream: ['api-routes', 'content-services'],
    monitoring: [
      { tool: '/admin', description: 'Group count and membership metrics' },
    ],
    links: [
  ...
  'crypto-services': {
    ...
    description: 'End-to-end encryption for group content',
    details: [
      'AES-256-GCM content encryption',
      'RSA-OAEP key wrapping (4096-bit)',
      'PBKDF2 passcode key derivation',
      'Group key management and rotation',
    ],
    files: [
      { path: 'app/lib/crypto/primitives.ts', description: 'AES, RSA, PBKDF2 operations' },
      { path: 'app/lib/crypto/groupKeys.ts', description: 'Group key generation and sharing' },
      { path: 'app/lib/crypto/contentEncryption.ts', description: 'Content encrypt/decrypt' },
      { path: 'app/contexts/CryptoContext.tsx', description: 'Crypto state management' },
      { path: 'app/services/keyExchangeService.ts', description: 'Key sharing between members' },
      { path: 'app/services/keyRotationService.ts', description: 'Manual key rotation' },
      { path: 'app/api/user-keys/route.ts', description: 'User key bundle CRUD' },
      { path: 'app/api/groups/[id]/keys/route.ts', description: 'Group key storage' },
    ],
    functions: [
      { name: 'encryptContent()', description: 'Encrypt Slate content with group key' },
      { name: 'decryptContent()', description: 'Decrypt Slate content with group key' },
      { name: 'deriveKeyFromPasscode()', description: 'PBKDF2 passcode to AES key' },
      { name: 'shareGroupKeyWithMember()', description: 'Wrap group key for new member' },
      { name: 'rotateGroupKey()', description: 'Generate new group key and re-share' },
    ],
    upstream: ['groups-services'],
    downstream: ['content-services'],
    monitoring: [],
    links: [
      { label: 'Encryption Settings', path: '/settings/security/encryption' },
    ],
  },
  'user-services': {
    title: 'User Services',
  ...
  'analytics-services': {
    ...
    files: [
      ...
      { path: 'app/services/StatsService.ts', description: 'Client-side statistics service' },
    ],
    functions: [
  ...
  'admin-api': {
    ...
    files: [
      ...
      { path: 'app/api/admin/payout-approvals/route.ts', description: 'List payout approval requests' },
      { path: 'app/api/admin/payout-approval/route.ts', description: 'Approve/reject payouts' },
    ...
    functions: [
      ...
      { name: 'getPendingApprovals()', description: 'Get payout approval queue' },
      { name: 'approveRejectPayout()', description: 'Process payout approval decision' },
    ],
